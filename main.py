...
class OCRService:
    ...
    def __init__(self):
        # Read the JSON credentials from Railway environment variable
        credentials_json = os.getenv("GOOGLE_APPLICATION_CREDENTIALS_JSON")
        
        if credentials_json:
            credentials_dict = json.loads(credentials_json)  # Convert JSON string to dictionary
        
            # Write JSON to a file in the `/app/` directory inside Railway
            credentials_path = "/app/service-account.json"
            with open(credentials_path, "w") as f:
                json.dump(credentials_dict, f)
        
            # Set the correct path for Google Cloud to find the file
            os.environ["GOOGLE_APPLICATION_CREDENTIALS"] = credentials_path
        
            # Load credentials and initialize Google Vision API client
